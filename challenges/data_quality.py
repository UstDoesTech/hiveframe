...
from ..exceptions import (
    ValidationError, SchemaError, ParseError, 
    EncodingError, NullValueError, DeadLetterQueue, DeadLetterRecord
)
        ...
        try:
            if expected_type == 'string':
                if not isinstance(value, str):
                    ...
                    raise ValidationError(f"Expected string for {field}", field=field,
                                        expected='string', actual=type(value).__name__)
                                        
            elif expected_type == 'int':
                if not isinstance(value, int) or isinstance(value, bool):
                    if self.auto_correct:
                        return int(float(value)), True
                    raise ValidationError(f"Expected int for {field}", field=field,
                                        expected='int', actual=type(value).__name__)
                                        
            elif expected_type == 'float':
                if not isinstance(value, (int, float)):
                    if self.auto_correct:
                        return float(value), True
                    raise ValidationError(f"Expected float for {field}", field=field,
                                        expected='float', actual=type(value).__name__)
                ...
                if not isinstance(value, bool):
                    ...
                    raise ValidationError(f"Expected bool for {field}", field=field,
                                        expected='bool', actual=type(value).__name__)
                                        
        except (ValueError, TypeError) as e:
            raise ValidationError(f"Cannot convert {field}: {e}", field=field,
                                expected=expected_type, actual=str(value)[:50])
...
def run_schema_drift_scenario(num_batches: int = 10, records_per_batch: int = 100) -> DataQualityResult:
    """
    ...
    - Renamed fields
    """
    logger.info("Starting schema drift scenario",
                num_batches=num_batches, records_per_batch=records_per_batch)
...
def run_unicode_encoding_scenario(num_records: int = 500) -> DataQualityResult:
    ...
    for i in range(num_records):
        ...
        records.append({
            'id': i,
            'text': sample,
            'description': f'Test record {i}: {sample}'
        })
