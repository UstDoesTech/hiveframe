...
def run_throughput_scaling_scenario(
    ...
    """
    logger.info("Starting throughput scaling scenario",
                initial=initial_records, max=max_records)
    ...
    while current_size <= max_records:
        ...
        try:
            results_data = hive.process(
                data,
                lambda item: (process_item(item), 1.0)
            )
...
def run_partition_skew_scenario(
    ...
    """
    logger.info("Starting partition skew scenario",
                num_records=num_records, skew_factor=skew_factor)
    ...
    results_data = hive.process(
        data,
        lambda item: process_with_partition_tracking(item)
    )
...
def run_sustained_load_scenario(
    ...
    logger.info("Starting sustained load scenario",
                rps=records_per_second, duration=duration_seconds)
    ...
    while time.time() - start_time < duration_seconds:
        ...
        batch = [
            {'id': record_id + i, 'value': random.random() * 1000}
    ...
    if throughput_samples:
        ...
        throughput_stddev = statistics.stdev(throughput_samples) if len(throughput_samples) > 1 else 0
        logger.info("Throughput stability",
                    avg=avg_throughput,
...
def run_worker_saturation_scenario(
    ...
    logger.info("Starting worker saturation scenario",
                num_records=num_records, worker_counts=worker_counts)
...
def run_all_scale_scenarios() -> Dict[str, List[ScaleResult]]:
    ...
    print("\n--- Sustained Load ---")
    results['sustained'] = [run_sustained_load_scenario(
    ...
    for category, category_results in results.items():
        ...
        for r in category_results:
            print(f"  {r.record_count:,} records: {r.throughput:,.0f} rec/s, "
