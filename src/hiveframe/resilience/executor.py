            ...
            try:
                ...
                if not self.circuit.allow_request():
                    raise CircuitOpenError(
                        f"Circuit for '{self.name}' is open", service_name=self.name
                    )
                ...
                while True:
                    try:
                        ...
                        delay = self.retry_policy.calculate_delay(state.attempt, state.last_delay)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "success_rate": (
                    self._success_count / self._call_count if self._call_count > 0 else 0
                ),
