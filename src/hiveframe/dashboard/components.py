...
class ColonyMetricsPanel:
    ...
    def update(self, metrics: Dict[str, Any]) -> None:
        ...
        self.temperature_history = [
            p for p in self.temperature_history 
            if p.timestamp.timestamp() > cutoff
        ]
        self.fitness_history = [
            p for p in self.fitness_history 
            if p.timestamp.timestamp() > cutoff
        ]
        self.throughput_history = [
            p for p in self.throughput_history 
            if p.timestamp.timestamp() > cutoff
        ]
...
class DanceFloorPanel:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'topPartitions': [
                {'partitionId': p, 'count': c}
                for p, c in self.top_partitions
            ],
            'recentDances': [
                {
            ...
            'queries': [
                {
                    ...
                    'duration': (q.end_time - q.start_time).total_seconds() 
                               if q.end_time else None,
