...
class ColonyState:
    ...
    def get_unassigned_sources(self) -> List[FoodSource]:
        ...
        with self._lock:
            return [s for s in self.food_sources.values() 
                    if s.assigned_worker is None]
                    
    def get_abandoned_sources(self) -> List[FoodSource]:
        """Get sources that should be abandoned and re-explored."""
        with self._lock:
            return [s for s in self.food_sources.values()
                    if s.is_abandoned(self.abandonment_limit)]
...
class Bee:
    ...
    def _onlooker_forage(self) -> Optional[WaggleDance]:
        ...
        best_partition = max(partition_scores.keys(),
...
class HiveFrame:
    ...
    def process(self,
                ...
                partition_fn: Optional[Callable[[Any], str]] = None) -> Dict[str, Any]:
        """
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            while cycles < self.max_cycles:
                ...
                if colony_temp > 0.8:
                    ...
                    self.colony.emit_pheromone(Pheromone(
                        signal_type='throttle',
                        intensity=colony_temp,
                        source_worker='colony'
                    ))
