...
class SLAOptimizer:
    ...
    def check_sla_compliance(self, sla: SLAMetrics) -> Dict[str, bool]:
        ...
        return {
            ...
            "overall": (
                sla.p95_response_time_ms <= self.target_p95_ms and
                sla.availability >= self.target_availability and
                sla.error_rate <= self.max_error_rate
            ),
        }
        ...
        if compliance["overall"]:
            ...
            if current_cost.resource_utilization < 0.4:
                ...
                recommendations.append(OptimizationRecommendation(
                    ...
                    risk_level="low" if strategy == OptimizationStrategy.AGGRESSIVE else "medium",
                    description=f"Reduce workers by {int(reduction*100)}% due to low utilization",
                    parameters={"worker_reduction": int(current_cost.active_workers * reduction)},
                ))
        else:
            ...
            if not compliance["response_time"]:
                recommendations.append(OptimizationRecommendation(
                    ...
                    parameters={"worker_increase": max(1, int(current_cost.active_workers * 0.3))},
                ))
...
class CostOptimizer:
    ...
    def optimize(self) -> Dict[str, Any]:
        ...
        total_savings = sum(r.estimated_savings_per_hour for r in recommendations if r.estimated_savings_per_hour > 0)
