...
class CostModel(ABC):
    ...
    @abstractmethod
    def estimate(self, node: PlanNode, stats: Dict[str, Statistics]) -> CostEstimate:
        """
...
class SwarmCostModel(CostModel):
    ...
    def __init__(self):
        ...
        # Adaptive weights
        self._weights = {"cpu": 1.0, "io": 10.0, "network": 5.0, "memory": 0.1}

    def estimate(self, node: PlanNode, stats: Dict[str, Statistics]) -> CostEstimate:
        """Estimate cost using swarm-inspired approach."""
        # Recursive estimation
        child_costs = [self.estimate(child, stats) for child in node.children]
    ...
    def _estimate_node(
        self, node: PlanNode, stats: Dict[str, Statistics], child_costs: List[CostEstimate]
    ) -> CostEstimate:
    ...
    def _estimate_scan(self, node: PlanNode, stats: Dict[str, Statistics]) -> CostEstimate:
    ...
    def _estimate_project(self, node: PlanNode, child_costs: List[CostEstimate]) -> CostEstimate:
    ...
    def _estimate_aggregate(self, node: PlanNode, child_costs: List[CostEstimate]) -> CostEstimate:
    ...
    def _estimate_sort(self, node: PlanNode, child_costs: List[CostEstimate]) -> CostEstimate:
        """Estimate sort cost."""
        input_rows = child_costs[0].estimated_rows if child_costs else 10000
    ...
    def _estimate_limit(self, node: PlanNode, child_costs: List[CostEstimate]) -> CostEstimate:
        """Estimate limit cost."""
    ...
    def _estimate_selectivity(self, predicates: List[Any], stats: Dict[str, Statistics]) -> float:
