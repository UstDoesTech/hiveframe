...
class SkewHandler:
    ...
    def split_skewed_partition(
        ...
        if num_splits is None:
            ...
            num_splits = max(
                2,
                math.ceil(partition_size / self.context.target_partition_size)
            )
            
        split_size = partition_size // num_splits
        
        return [
            (f"{key}_split_{i}", split_size)
...
class AdaptiveQueryExecutor:
    ...
    def get_optimal_join_strategy(
        self,
        left_stats: RuntimeStatistics,
        right_stats: RuntimeStatistics
    ) -> JoinStrategy:
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'adaptations_by_type': defaultdict(
                int,
                {a['type']: 1 for a in adaptations}
            ),
