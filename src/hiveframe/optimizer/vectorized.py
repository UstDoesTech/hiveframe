...
class VectorBatch:
    ...
    def from_rows(
        ...
        schema: Optional[Dict[str, VectorType]] = None
    ) -> 'VectorBatch':
    ...
    def select(self, columns: List[str]) -> 'VectorBatch':
        ...
        return VectorBatch(
            ...
            columns={
                ...
                for name in columns if name in self.columns
            },
            null_masks={
                name: self.null_masks[name]
                for name in columns if name in self.null_masks
            },
            types={
                name: self.types[name]
                for name in columns if name in self.types
            }
        )
    ...
    def filter(self, mask: List[bool]) -> 'VectorBatch':
        ...
        return VectorBatch(
            ...
            null_masks={
                name: [mask_vals[i] for i in indices]
                for name, mask_vals in self.null_masks.items()
            },
            types=self.types.copy()
        )
        ...
        for out_name, expr in self.expressions.items():
            if isinstance(expr, str):
                ...
                if expr in batch.columns:
                    ...
                    new_null_masks[out_name] = batch.null_masks.get(expr, [False] * batch.num_rows).copy()
...
class VectorizedAggregate(VectorizedOp):
    ...
    def execute(self, batch: VectorBatch) -> VectorBatch:
        ...
        for i in range(batch.num_rows):
            key = tuple(
                batch.columns[col][i] if col in batch.columns else None
...
class VectorizedSort(VectorizedOp):
    ...
    def __init__(
        self,
        sort_keys: List[Tuple[str, bool]]  # (column, ascending)
    ):
    ...
    def execute(self, batch: VectorBatch) -> VectorBatch:
        ...
        return VectorBatch(
            ...
            null_masks={
                name: [mask[i] for i in indices]
                for name, mask in batch.null_masks.items()
            },
...
class ParallelVectorizedExecutor:
    ...
    def execute(
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            futures = {
                executor.submit(pipeline.execute_batch, batch): i
