...
from .rules import (
    PlanNode, NodeType, OptimizationRule, 
    get_default_rules, PredicatePushdown, ProjectionPruning
)
...
class QueryOptimizer:
    ...
    def optimize(self, 
                 plan: PlanNode,
                 stats: Optional[Dict[str, Statistics]] = None) -> OptimizedPlan:
        """
        ...
        best_candidate, iterations, explored = self._swarm_optimize(
            current_plan, stats
        )
    ...
    def _apply_rule_recursive(self, 
                              node: PlanNode, 
                              rule: OptimizationRule) -> PlanNode:
        """Apply a rule recursively to plan tree."""
        # Apply to children first (bottom-up)
        new_children = [
            self._apply_rule_recursive(child, rule)
    ...
    def _initialize_swarm(self,
                         plan: PlanNode,
                         stats: Dict[str, Statistics]) -> List[PlanCandidate]:
    ...
    def _generate_neighbor(self,
                          plan: PlanNode,
                          stats: Dict[str, Statistics]) -> PlanNode:
        ...
        if joins:
            ...
            if len(join_node.children) >= 2:
                ...
                join_node.children[0], join_node.children[1] = \
                    join_node.children[1], join_node.children[0]
