...
class TrackedTask:
    ...
    def any_completed(self) -> bool:
        ...
        return any(
            ex.end_time is not None and ex.error is None
            for ex in self.executions
        )
                ...
                for ex in task.executions:
                    if ex.execution_id == execution_id:
                        ...
                        if error:
                            ...
                            if ex.duration:
                                self._completed_times.append(
                                    (time.time(), ex.duration)
                                )
                                # Keep only recent times
                                cutoff = time.time() - self.config.stats_window
                                self._completed_times = [
                                    (ts, dur) for ts, dur in self._completed_times
                                    if ts > cutoff
                                ]
    ...
    def detect_slow_tasks(self) -> List[TrackedTask]:
        ...
        for task in self.tracker.get_running_tasks():
            ...
            if (
                ...
                elapsed > self.config.min_speculation_delay and
                task.speculation_count < self.config.max_speculation_attempts and
                task.state != TaskState.SPECULATING
            ):
...
class SpeculativeExecutor:
    ...
    def start(self) -> None:
        ...
        if self.config.enabled:
            self._speculation_thread = threading.Thread(
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'speculation_success_rate': speculative_wins / total_completed if total_completed > 0 else 0,
