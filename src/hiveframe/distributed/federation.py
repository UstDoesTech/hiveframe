...
class FederatedHive:
    ...
    def is_available(self) -> bool:
        ...
        return (
            self.health in (HiveHealth.HEALTHY, HiveHealth.DEGRADED) and
            self.current_load < 0.95 and
...
class HiveRegistry:
    ...
    def register(self, hive: FederatedHive) -> None:
        ...
        with self._lock:
            ...
            self._gossip_state[hive.hive_id] = {
                'version': 1,
                'timestamp': time.time()
            }
...
class FederationCoordinator:
    ...
    def announce_task(
        ...
        with self._lock:
            ...
            cutoff = time.time() - 60
            self._pending_dances = [
                d for d in self._pending_dances
    ...
    def select_target_hive(
        ...
        available = [
            h for h in self.registry.get_available_hives()
            if h.hive_id not in exclude
        ]
    ...
    def balance_load(self) -> Dict[str, List[str]]:
        ...
        for over_hive in overloaded:
            ...
            candidates = [
                h for h in underloaded
                if h.region == over_hive.region
            ]
            if not candidates:
                candidates = underloaded
                
            if candidates:
                recommendations[over_hive.hive_id] = [
                    c.hive_id for c in candidates[:3]
                ]
...
class HiveFederation:
    ...
    def route_task(
        ...
        if (
            self.local_hive.is_available and
            self.local_hive.current_load < 0.7 and
            (not locality_hints or 
             self.local_hive.datacenter in locality_hints or
             self.local_hive.region in locality_hints)
        ):
