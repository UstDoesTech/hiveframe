...
class DataLocality:
    ...
    def get_best_location(
        self,
        target_datacenter: Optional[str] = None,
    ...
    def schedule(
        ...
        with self._lock:
            ...
            if available_nodes and best_dc in available_nodes:
                ...
                for hint in hints:
                    for loc in hint.locations:
                        if (loc.get('datacenter') == best_dc and 
                            loc.get('node') in available_nodes[best_dc]):
...
class CrossDatacenterManager:
    ...
    def suggest_replications(
        self,
        replication_threshold: float = 0.3
    ) -> List[Dict[str, Any]]:
        """
