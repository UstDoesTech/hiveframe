...
class ColonyHealthMonitor:
    ...
    def __init__(
        ...
        self._worker_gauge = self._registry.gauge("workers_by_status", "Workers by health status")
    ...
    def _collect_health(self) -> ColonyHealthReport:
        ...
        if throttle_level > 0.5:
            alerts.append(
                f"WARNING: High throttle pheromone ({throttle_level:.2f}) - backpressure active"
            )

        alarm_level = self.colony.sense_pheromone("alarm")
        if alarm_level > 0.3:
            alerts.append(
                f"WARNING: Alarm pheromones detected ({alarm_level:.2f}) - errors occurring"
            )
