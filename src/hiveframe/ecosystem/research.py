...
class GrantApplication:
    ...
    def submit_grant(
        ...
        grant_id = hashlib.sha256(
            f"{researcher_id}{title}{time.time()}".encode()
        ).hexdigest()[:16]
    ...
    def add_review(
        ...
        if len(self.reviews[grant_id]) >= 3:  # Require 3 reviews
            avg_score = sum(r["score"] for r in self.reviews[grant_id]) / len(self.reviews[grant_id])
...
class ResearchProjectTracker:
    ...
    def create_project(
        ...
        project_id = hashlib.sha256(
            f"{grant_id}{title}{time.time()}".encode()
        ).hexdigest()[:16]
    ...
    def get_tracking_stats(self) -> Dict:
        ...
        completed = sum(
            ...
            if p.phase == ResearchPhase.COMPLETED
        )
...
class PublicationManager:
    ...
    def add_publication(
        ...
        pub_id = hashlib.sha256(
            f"{project_id}{title}{time.time()}".encode()
        ).hexdigest()[:16]
