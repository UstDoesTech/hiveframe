...
class EdgeNode:
    ...
    def can_accept_task(self) -> bool:
        ...
        return self.is_online and self.battery_level > 0.2 and self.current_load < self.capacity
...
class EdgeNodeManager:
    ...
    def assign_task_to_edge(
        ...
        candidates = [node for node in self.nodes.values() if node.can_accept_task()]
        ...
        if preferred_location:
            location_candidates = [n for n in candidates if n.location == preferred_location]
...
class OfflineOperationSupport:
    ...
    def get_offline_stats(self) -> Dict:
        ...
        total_pending = sum(len(ops) for ops in self.pending_operations.values())
