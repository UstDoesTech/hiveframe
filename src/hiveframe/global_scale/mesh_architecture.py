...
class GlobalMeshCoordinator:
    ...
    def route_task(
        ...
        available_regions = [
            ...
            if r.status in (RegionStatus.ACTIVE, RegionStatus.DEGRADED)
            and r.utilization() < 0.95
        ]
        ...
        if data_locality:
            local_regions = [
                ...
                if r.region_id in data_locality
            ]
            if local_regions:
    ...
    def evaporate_pheromones(self) -> None:
        ...
        for region in self.regions.values():
            region.pheromone_level = max(
                0.5,
                region.pheromone_level * self.pheromone_decay_rate
            )
