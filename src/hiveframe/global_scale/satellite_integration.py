...
class HighLatencyProtocol:
    ...
    def send_message(
        ...
        message_id = hashlib.sha256(f"{link_id}{time.time()}{message}".encode()).hexdigest()[:16]
    ...
    def process_queue(self) -> List[Dict]:
        ...
        sorted_messages = sorted(self.message_queue, key=lambda m: m["priority"], reverse=True)
...
class BandwidthOptimizer:
        ...
        for transfer in self.transfer_queue:
            if transfer["transfer_id"] == transfer_id:
                self.current_usage_kbps = max(
                    0, self.current_usage_kbps - transfer["actual_size_kb"]
                )
                break
    ...
    def get_bandwidth_stats(self) -> Dict:
        ...
        utilization = (
            self.current_usage_kbps / self.max_bandwidth_kbps if self.max_bandwidth_kbps > 0 else 0
        )
...
class DataBufferingStrategy:
    ...
    def get_buffer_stats(self) -> Dict:
        ...
        utilization = self.current_buffer_mb / self.max_buffer_mb if self.max_buffer_mb > 0 else 0
