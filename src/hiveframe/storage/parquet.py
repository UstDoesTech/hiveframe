...
from .formats import (
    ...
    PartitionSpec, FileMetadata, ColumnStats
)
...
class ParquetWriter:
    ...
    def _write_native(self) -> FileMetadata:
        ...
        output = {
            ...
            'row_count': len(all_records),
            'row_groups': [
                {'data': group} for group in self._row_groups
            ],
            'metadata': {
...
class ParquetReader:
    ...
    def read(self,
             ...
             filter: Optional[Any] = None) -> HiveDataFrame:
        """
    ...
    def _read_with_pyarrow(self,
                          columns: Optional[List[str]],
                          filter: Optional[Any]) -> HiveDataFrame:
        """Read using pyarrow."""
        import pyarrow.parquet as pq
...
def read_parquet(path: str,
                 columns: Optional[List[str]] = None,
                 options: Optional[StorageOptions] = None) -> HiveDataFrame:
    """
    Read Parquet file to HiveDataFrame.
...
def write_parquet(data: Union[List[Dict], HiveDataFrame],
                  path: str,
                  schema: Optional[ParquetSchema] = None,
                  options: Optional[StorageOptions] = None) -> FileMetadata:
    """
    Write data to Parquet file.
