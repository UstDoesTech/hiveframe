...
class PartitionSpec:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize to dictionary."""
        return {"spec-id": self.spec_id, "fields": [f.to_dict() for f in self.fields]}

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "PartitionSpec":
        """Deserialize from dictionary."""
        return cls(
            spec_id=data["spec-id"], fields=[PartitionField.from_dict(f) for f in data["fields"]]
        )
...
class IcebergTable:
    ...
    def create(
        ...
        # Create schema
        iceberg_schema = IcebergSchema(schema_id=0, fields=schema)
    ...
    def as_of(
        self, snapshot_id: Optional[int] = None, timestamp_ms: Optional[int] = None
    ) -> "IcebergTable":
        """
...
class SchemaEvolution:
    ...
    def add_column(
        self, name: str, data_type: str, required: bool = False, doc: Optional[str] = None
    ) -> "SchemaEvolution":
        """Add a new column."""
        self._changes.append(
            {"action": "add", "name": name, "type": data_type, "required": required, "doc": doc}
        )
        return self

    def rename_column(self, old_name: str, new_name: str) -> "SchemaEvolution":
        """Rename an existing column."""
        self._changes.append({"action": "rename", "old_name": old_name, "new_name": new_name})
        return self

    def drop_column(self, name: str) -> "SchemaEvolution":
        """Mark column as dropped (makes it nullable and hidden)."""
        self._changes.append({"action": "drop", "name": name})
        return self

    def make_optional(self, name: str) -> "SchemaEvolution":
        """Make a required column optional."""
        self._changes.append({"action": "make_optional", "name": name})
        return self

    def update_doc(self, name: str, doc: str) -> "SchemaEvolution":
        """Update column documentation."""
        self._changes.append({"action": "update_doc", "name": name, "doc": doc})
        return self
...
def write_iceberg(
    ...
    if not os.path.exists(path):
        ...
        table = IcebergTable.create(path=path, schema=fields, partition_by=partition_spec)
    else:
        table = IcebergTable.load(path)
