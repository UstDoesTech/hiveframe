...
class DeltaTransaction:
    ...
    def remove_file(self, path: str, partition_values: Optional[Dict[str, str]] = None) -> None:
    ...
    def commit(self, operation: str = "WRITE") -> int:
        ...
        commit = CommitInfo(
            ...
            operation_metrics={
                ...
                "numOutputBytes": str(sum(a.get("add", {}).get("size", 0) for a in self.actions)),
            },
        )
...
class DeltaTable:
    ...
    def create(
        ...
        commit = CommitInfo(timestamp=int(time.time() * 1000), operation="CREATE TABLE")

        actions = [protocol.to_dict(), metadata.to_dict(), commit.to_dict()]
    ...
    def write(
        ...
        with self.start_transaction() as txn:
            ...
            txn.add_file(file_name, size=metadata.size_bytes, stats=json.dumps(stats))
    ...
    def to_dataframe(
        self, version: Optional[int] = None, timestamp: Optional[datetime] = None
    ) -> HiveDataFrame:
        """
    ...
    def optimize(self) -> Dict:
        ...
        return {"filesCompacted": len(small_files), "recordsProcessed": len(all_records)}


def read_delta(path: str, version: Optional[int] = None) -> HiveDataFrame:
    """
