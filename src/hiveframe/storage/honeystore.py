...
class DictionaryEncoder:
    ...
    def encode(values: List[Any]) -> Tuple[bytes, bytes]:
        ...
        if max_idx < 256:
            ...
            indices = b''.join(
                struct.pack('<H', value_to_idx.get(v, max_idx))
                for v in values
            )
    ...
    def decode(dictionary: bytes, indices: bytes, use_short: bool = False) -> List[Any]:
        ...
        return [
            unique_values[idx] if idx < len(unique_values) else None
            for idx in idx_list
        ]
...
class HoneyStoreWriter:
    ...
    def _infer_schema(self, rows: List[Dict[str, Any]]) -> None:
        ...
        self.schema = [
            ColumnMetadata(name=name, data_type=dtype)
...
class HoneyStoreReader:
    ...
    def _read_metadata(self) -> None:
        ...
        with open(self.path, 'rb') as f:
            ...
            self._metadata = HoneyStoreMetadata.from_dict(
        ...
        with open(self.path, 'rb') as f:
            ...
            while True:
                ...
                for col_name, encoded in column_data.items():
                    ...
                    col_meta = next(
                        (c for c in self.schema if c.name == col_name),
                        None
                    )
                    if col_meta is None:
                        continue
                    ...
                    if col_meta.encoding == EncodingType.DICTIONARY:
                        ...
                        decoded_columns[col_name] = DictionaryEncoder.decode(
                            dict_data, indices
                        )
                    elif col_meta.encoding == EncodingType.RLE:
                        decoded_columns[col_name] = RLEEncoder.decode(
                            encoded, col_meta.data_type
                        )
