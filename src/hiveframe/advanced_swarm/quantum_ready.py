    ...
    def optimize(
        ...
        for iteration in range(max_iterations):
            ...
            classical_population = [
                self.observe(ind) for ind in self.quantum_population
            ]
            ...
            if best_solution is not None:
                self.quantum_population = [
                    self.quantum_gate_update(ind, best_solution)
...
class HybridQuantumClassical:
    ...
    def _estimate_gradient(self, hamiltonian: List[List[float]], params: List[float]) -> List[float]:
