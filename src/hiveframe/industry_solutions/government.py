...
class SecureMultiTenancy:
    ...
    def get_multitenancy_stats(self) -> Dict:
        ...
        unauthorized_attempts = sum(
            1 for log in self.access_log if log["action"] == "unauthorized_access_attempt"
        )
...
class ComplianceFramework:
    ...
    def get_compliance_status(
        ...
        entity_checks = [c for c in self.compliance_checks if c["entity_id"] == entity_id]
        ...
        for check in entity_checks:
            ...
            if (
                standard not in by_standard
                or check["timestamp"] > by_standard[standard]["timestamp"]
            ):
