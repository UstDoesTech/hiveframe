...
class TestPartitionSplitter:
    ...
    def test_can_split(self):
        ...
        small_partition = Partition(
            partition_id='small',
            data=list(range(5))
        )
        
        large_partition = Partition(
            partition_id='large',
            data=list(range(50))
        )
    ...
    def test_split_by_fitness(self):
        ...
        left, right = splitter.split_by_fitness(
            partition,
            fitness_fn=lambda x: x / 20.0
        )
...
class TestLocalityAwareScheduler:
    ...
    def test_schedule_with_locality(self):
        ...
        scheduler.register_data(
            data_id='data-1',
            locations=[{'datacenter': 'us-east-1a', 'region': 'us-east-1'}]
        )
