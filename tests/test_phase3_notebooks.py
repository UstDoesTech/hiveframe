...
class TestNotebookSession:
    ...
    def test_execute_python_cell(self):
        ...
        execution = session.execute_cell("x = 42", language=KernelLanguage.PYTHON)
...
class TestCollaborationManager:
    ...
    def test_add_user(self):
        ...
        user = collab.add_user(session.session_id, username="alice", email="alice@example.com")
    ...
    def test_get_operations_since(self):
        ...
        collab.broadcast_operation(
            session.session_id, user.user_id, "cell_1", OperationType.INSERT, 0, "code1"
        )
        collab.broadcast_operation(
            session.session_id, user.user_id, "cell_1", OperationType.INSERT, 5, "code2"
        )
...
class TestNotebookFormat:
    ...
    def test_write_read_notebook(self):
        ...
        notebook.cells.append(
            NotebookCell(cell_type=CellType.CODE, source=["print('Hello, HiveFrame!')"])
        )
        notebook.cells.append(NotebookCell(cell_type=CellType.MARKDOWN, source=["# Test Notebook"]))
