...
def demo_level_1_beginner():
    ...
    # Group by department
    dept_stats = df.groupBy("department").agg(avg(col("salary")), count(col("salary")))
...
def demo_level_2_intermediate():
    ...
    category_stats = df.groupBy("category").agg(
        sum_agg(col("amount")), avg(col("amount")), count(col("id"))
    )
    ...
    # Status breakdown
    status_stats = df.groupBy("status").agg(count(col("id")))
...
def demo_level_3_advanced():
    ...
    retry_policy = RetryPolicy(
        max_retries=3, base_delay=0.01, strategy=BackoffStrategy.EXPONENTIAL, jitter=True
    )
        ...
        try:
            ...
            if random.random() < 0.05:
                raise ValidationError(
                    f"Invalid record {i}", field="id", expected="valid", actual="corrupted"
                )
            processed += 1
    ...
    circuit = CircuitBreaker(
        "demo_service", CircuitBreakerConfig(failure_threshold=5, success_threshold=2, timeout=1.0)
    )
...
def demo_level_4_expert():
    ...
    print(
        f"Worker distribution: {len(workers['employed'])} employed, "
    ...
    for load in load_levels:
        ...
        if load > 0.8:
            colony.emit_pheromone(
                Pheromone(signal_type="throttle", intensity=load, source_worker="pressure_test")
            )

        throttle = colony.sense_pheromone("throttle")
        status = (
            "🔴 THROTTLING" if throttle > 0.5 else "🟡 Warning" if throttle > 0.2 else "🟢 Normal"
        )
...
def demo_level_5_production():
    ...
    for op, stats in list(profile_stats.items())[:5]:
        if stats["count"] > 0:
            print(f"  {op}: {stats['count']} calls, " f"avg={stats['mean']*1000:.2f}ms")
