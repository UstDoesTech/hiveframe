...
def demo_dataframe_api():
    ...
    start = time.time()
    result = df.filter((col("category") == "Electronics") & (col("amount") > 500)).select(
        "id", "amount", "region", "customer_id"
    )
    elapsed = time.time() - start
    ...
    summary = df.groupBy("category").agg(
        count(col("id")), sum_agg(col("amount")), avg(col("amount")), max_agg(col("amount"))
    )
    elapsed = time.time() - start
    ...
    regional = (
        ...
        .agg(count(col("id")), sum_agg(col("amount")))
        .orderBy("category")
    )
    elapsed = time.time() - start
    ...
    start = time.time()
    enriched = (
        df.withColumn("total_value", col("amount") * col("quantity"))
...
def demo_streaming():
    ...
    stream = HiveStream(
        num_workers=6, buffer_size=1000, employed_ratio=0.5, onlooker_ratio=0.3, scout_ratio=0.2
    )
...
def demo_advanced_streaming():
    ...
    for i in range(30):
        record = StreamRecord(
            key=f"metric_{i % 2}", value=random.random() * 100, timestamp=time.time()
        )
        sliding_processor.process_record(
            record, aggregator=lambda acc, v: acc + v, initial_value=0.0
        )
        time.sleep(0.05)
