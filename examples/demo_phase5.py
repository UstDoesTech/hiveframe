...
from hiveframe.industry_solutions.manufacturing import SensorDataProcessor, SensorType, SensorReading
from hiveframe.industry_solutions.government import DataSovereigntyController, DataAsset, JurisdictionLevel, SecurityLevel
...
def demo_open_ecosystem():
    ...
    for researcher_id, institution, title, amount in proposals:
        grant_id = grants.submit_grant(
            researcher_id, institution, title,
            f"Research on {title.lower()}", amount
        )
